...
import json
import stripe
from .models import Room, Payment, ChatAccess, Message, UserProfile, Owner, Client, RoomAccess, ClientPayment
...
def get_messages(request):
    ...
    if hasattr(request.user, 'client'):
        ...
        other_user = None  # Will show all clients who messaged
    else:
...
def send_message(request):
    ...
    try:
        ...
        if hasattr(request.user, 'client'):
            ...
            # For owners, find the client who messaged about this room
            latest_client_message = Message.objects.filter(
                room=room,
                receiver=request.user
            ).order_by('-timestamp').first()
            
            if latest_client_message:
                receiver = latest_client_message.sender
            else:
                return JsonResponse({'error': 'No client conversation found'}, status=400)
        else:
            return JsonResponse({'error': 'Access denied'}, status=403)
    ...
    for msg in messages:
        ...
        if latest_message:
            ...
            if latest_message.sender != request.user:
                client_name = latest_message.sender.get_full_name() or latest_message.sender.username
            else:
                client_name = latest_message.receiver.get_full_name() or latest_message.receiver.username
